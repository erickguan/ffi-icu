  ...
  module Lib
    ...
    def self.load_icu
      ...
      if not lib_names or lib_names.length == 0
        raise LoadError,
              "Could not find ICU on #{ICU.platform.inspect}. Patches welcome, or you can add the containing directory yourself: #{self}.search_paths << '/path/to/lib'"
      end
    ...
    attach_function :udat_open, "udat_open#{suffix}",
                    [:date_format_style, :date_format_style, :string, :pointer, :int32_t, :pointer, :int32_t, :pointer], :pointer
