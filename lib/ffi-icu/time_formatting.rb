...
module ICU
  module TimeFormatting
    ...
    def self.format(dt, options = {})
      p 'before creation'
      formatter = create(@default_options.merge(options))
      p "created formatter"
      formatter.format(dt)
    end

    class BaseFormatter
      BASE_SKELETON_PATTERN_POINTER_SIZE = 10

      def set_attributes(options)
        options.each { |key, value| Lib.unum_set_attribute(@formatter, key, value) }
        self
      end
      ...
      def make_formatter(time_style, date_style, locale, time_zone_str, skeleton)
        ...
        tz_len      = 0
        pattern_len = -1
        pattern_ptr = FFI::MemoryPointer.new(BASE_SKELETON_PATTERN_POINTER_SIZE)
    ...
    class DateTimeFormatter < BaseFormatter
      ...
      def initialize(options = {})
        ...
        if @hour_cycle && !HOUR_CYCLE_SHORT_SYMBOLS.keys.include?(@hour_cycle)
          raise ICU::Error.new("Unknown hour cycle #{@hour_cycle}")
        end

        @formatter = make_formatter(time_style, date_style, @locale, time_zone, skeleton)
      ...
      def parse(str)
        ...
        str_l = str.size
        Lib.check_error do |error|
          ret = Lib.udat_parse(@formatter, str_u, str_l, nil, error)
          Time.at(ret / 1000.0)
        end
      end
      ...
      def format(dt)
        ...
        begin
          Lib.check_error do |error|
            case dt
            when Date
              needed_length = Lib.udat_format(@formatter, Time.mktime(dt.year, dt.month, dt.day, 0, 0, 0, 0).to_f * 1000.0,
                                              out_ptr, needed_length, nil, error)
            when Time
              needed_length = Lib.udat_format(@formatter, dt.to_f * 1000.0, out_ptr, needed_length, nil, error)
            end
          end

          out_ptr.string.strip
        rescue BufferOverflowError
          raise BufferOverflowError, "needed: #{needed_length}" if retried

          needed_length += C_NUL_TERMINATOR_SIZE
      ...
      def date_format(localized = true)
        ...
        retried = false
        p needed_length, retried, "date_format"

        begin
          Lib.check_error do |error|
            needed_length = Lib.udat_toPattern(@formatter, localized, out_ptr, needed_length, error)
          end
      ...
      def replace_hour_symbol!
        ...
        skeleton_str.gsub!(/[hHkKjJ]/, HOUR_CYCLE_SHORT_SYMBOLS[@hour_cycle])
        ...
        if HOUR_CYCLE_SHORT_SYMBOLS.keys.include?(resolved_hour_cycle)
          new_pattern_str.gsub!(/[hHkK](?=(?:[^\']|\'[^\']*\')*$)/, HOUR_CYCLE_SHORT_SYMBOLS[resolved_hour_cycle])
        end
      ...
      # udat_getLocaleByType to look it up from @formatter, because the latter will throw away
      ...
      def current_pattern_as_uchar
        Lib::Util.read_uchar_buffer_as_ptr(0) do |buf, error|
          Lib.udat_toPattern(@formatter, false, buf, buf.length_in_uchars, error)
        end
      end
      ...
      def set_date_format_impl(localized, pattern_str)
        ...
        Lib.check_error do |error|
          return Lib.udat_applyPattern(@formatter, localized, pattern, pattern_len)
        end
      end
