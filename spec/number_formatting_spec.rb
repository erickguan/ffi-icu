module ICU
  module NumberFormatting
    describe 'NumberFormatting' do
      it 'formats a simple integer' do
      ...
      it 'spells numbers' do
        expect(NumberFormatting.spell('en_US', 1_000)).to(eq('one thousand'))
        expect(NumberFormatting.spell('de-DE',
                                      123.456)).to(
                                        eq("ein\u{AD}hundert\u{AD}drei\u{AD}und\u{AD}zwanzig Komma vier fünf sechs")
                                      )
      end

      it 'is able to re-use number formatter objects' do
      ...
      it 'is able to re-use currency formatter objects' do
        curf = NumberFormatting.create('en-US', :currency)
        expect(curf.format(1_000.12, 'USD')).to(eq('$1,000.12'))
      end

      it 'allows for various styles of currency formatting if the version is new enough' do
